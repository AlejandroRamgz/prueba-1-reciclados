...
def crear_informe():
    if request.method == 'POST':
        ...
        #cantidad_personas = int(request.form.get('cantidad_personas'))         3333333333333333333333333333333333333333333333333
        #nombres_personas = []
        #cargos_personas = []

        #for i in range(cantidad_personas):                                    33333333333333333333333333333333333333333
        #    nombre_persona = request.form.get(f'nombre_persona_{i}')
        #    cargo_persona = request.form.get(f'cargo_persona_{i}')
        #    nombres_personas.append(nombre_persona)
        #    cargos_personas.append(cargo_persona)

        #imagen_0 = request.files['imagen_0']
        ...
        for i in range(4):
            ...
            if imagen:
                ...
                ## (Aquí debes incluir tu lógica para guardar la imagen en un archivo temporal)
                #nombre_temporal = f'temp_{i}.jpg'
                #imagen.save(nombre_temporal)
                #
                ## Obtener las coordenadas de la celda actual
                #x = pdf.get_x()
                #y = pdf.get_y()
                #
                ## Agregar la imagen al PDF en la posición especificada
                #pdf.image(nombre_temporal, x=x, y=y, w=47.5, h=55)
                #
                ## Avanzar a la siguiente celda
                #pdf.cell(w=47.5, h=55, txt="", border=1, align='C', fill=0)

        #for i, persona in enumerate(personas, 1):
        #    text += f'Persona {i}: Nombre: {persona["nombre"]}, Cargo: {persona["cargo"]}\n'

        #Lista de personas:
        
        #c.drawString(1 * inch, 10.5 * inch, text)

        # Escribir los datos de cada persona en el PDF
        #y = 9.5
        #for persona in personas:
        #    text = f"Nombre: {persona['nombre']}, Cargo: {persona['cargo']}"
        ...
        #    pdf.cell(w = 14, h = 5, txt = str(valor[0]), border = 1, align = 'C', fill = 0)
        #    pdf.cell(w = 40, h = 5, txt = valor[1], border = 1, align = 'C', fill = 0)
        #    pdf.cell(w = 55, h = 5, txt = valor[2], border = 1, align = 'C', fill = 0)
        #    pdf.multi_cell(w = 0, h = 5, txt = valor[3], border = 1, align = 'L', fill = 0)
